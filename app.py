...
class HttpWSSProtocol(websockets.WebSocketServerProtocol):
    ...
    async def http_handler(self, method, path, version):
        ...
        try :
            ...
            if 'is' not in RequestJson['query'].values() and 'what' not in RequestJson['query'].values() and 'are' not in RequestJson['query'].values():
   ...
   #             jsonRequest = {"object": obj.lower(), "value": value, "query": "cmd"}
                if 'value' not in RequestJson['Switch_State'].keys():
                    ...
                    if 'value' in RequestJson['scene'].keys():
                        ...
                        print({"object":"switch","value":scene,"query":"scene"})
                        jsonRequest = {"object": "switch", "value": scene, "query": "scene"}
                    else:
